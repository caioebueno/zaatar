...
export type ResolveChatwootConversationInput = {
  businessId?: string;
  conversationId: string;
  query: Record<string, string | undefined>;
};

export class ResolveChatwootConversationUseCase {
  constructor(
    private readonly gateway: ChatwootProxyGateway,
    private readonly branchRepository: BranchChatwootConfigRepository,
  ) {}

  async execute(input: ResolveChatwootConversationInput): Promise<unknown> {
    ...
    const response = await this.gateway.resolveConversation({
