...
const ProductModal: React.FC<TProductModal> = ({
  ...
  return (
    <Dialog open={product !== null} onOpenChange={onClose}>
      <DialogContent showCloseButton={false} className="w-full h-full p-0 bg-foreground transition-all">
        {product && (
          <div>
            <div>
              <DialogClose className="absolute top-4 left-4 p-3 rounded-full bg-background"><FiArrowLeft size={18} /></DialogClose>
              {product.photos && product.photos[0] && <img src={product.photos[0]?.url} alt={`${product.name} photo`} className="h-100 object-cover" />}
            </div>
            ...
            <div className="absolute bottom-0 py-5 px-4 bg-background w-full flex flex-row items-center justify-between">
              <QuantitySelector onChange={(value) => setQuantity(value)} value={quantity} />
              <Button onClick={() => onAdd(product.id, quantity)} className="text-[16px] font-bold bg-brandBackground py-3 px-8 leading-5">Add</Button>
            </div>
          </div>
        )}
      </DialogContent>
...
function QuantitySelector({
  ...
  return (
    <div className="inline-flex items-center">
      ...
      <span className={`${small ? 'min-w-6' : 'min-w-10'} text-center text-lg font-medium`}>{value}</span>
