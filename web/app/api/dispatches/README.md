...
Returns the next pending dispatch for the provided driver (including dispatches not marked as dispatched yet), as long as there are undelivered orders. Prioritizes by `queueIndex`, then by dispatch time/creation time. Returns `null` when no dispatch is available.
