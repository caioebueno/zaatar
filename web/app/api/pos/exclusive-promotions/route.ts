...
export async function GET(request: NextRequest) {
  try {
    ...
    // Force timezone validation early so request errors are explicit.
    ...
    const payload = promotions
      .map((promotion) => {
        ...
        return {
          ...
          active: promotion.active,
          expireAt: promotion.expireAt
            ? promotion.expireAt.toISOString()
            : null,
          validWeekdays: promotion.validWeekdays,
          availableNow,
    ...
    console.error("GET /api/pos/exclusive-promotions error:", error);
