-- Reconcile production drift without data loss.
-- This migration is intentionally idempotent.

-- 1) Ensure ComboProductItem exists with indexes and FKs.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'ComboProductItem_comboId_fkey'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'ComboProductItem_productId_fkey'
  ) THEN
    ALTER TABLE "ComboProductItem"
...
-- 2) Match updatedAt defaults seen in production inventory tables.
